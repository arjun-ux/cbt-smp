...
const fetchData = async () => {
  ...
  try {
    ...
    alertStore.showAlert("Gagal memuat data mata pelajaran", "error")
  } finally {
...
const filteredItems = computed(() => {
  ...
  return items.value.filter(i => 
    i.nama_mapel.toLowerCase().includes(q) || i.kode_mapel.toLowerCase().includes(q)
  )
})
...
const saveItem = async () => {
  isSaving.value = true
  try {
    const url = isEditMode.value ? `/api/admin/mapel/${editId.value}` : '/api/admin/mapel'
    ...
    if(res.ok) {
      alertStore.showAlert(isEditMode.value ? "Data diperbarui" : "Mata pelajaran berhasil ditambahkan", "success")
...
const executeDelete = async () => {
  ...
  try {
    if (deleteTargetId.value) {
      const res = await fetch(`/api/admin/mapel/${deleteTargetId.value}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${authStore.token}` }
      })
      if (res.ok) {
...
<template>
  <div class="space-y-6 pb-12">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-100 gap-4">
      <div>
        <h3 class="text-xl font-bold text-slate-800">Manajemen Mata Pelajaran</h3>
        <p class="text-sm text-slate-500 mt-1">Kelola daftar mata pelajaran yang diujikan</p>
      </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      <div class="p-4 border-b border-slate-50 bg-slate-50/30 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div class="relative w-full sm:w-80">
          ...
          <input v-model="searchQuery" type="text" placeholder="Cari kode atau nama mapel..." class="block w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 transition-all">
        </div>
        ...
        <div v-else class="text-xs font-bold text-slate-400 bg-slate-100 px-3 py-1.5 rounded-lg uppercase tracking-wider">
          Total: {{ filteredItems.length }} Mapel
        </div>
      </div>
        ...
        <table class="min-w-full divide-y divide-slate-100">
          <thead class="bg-slate-50">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Nama Mata Pelajaran</th>
          ...
          <tbody class="bg-white divide-y divide-slate-50">
            ...
            <tr v-else-if="filteredItems.length === 0">
              <td colspan="4" class="px-6 py-12 text-center text-slate-400 italic">Data mata pelajaran tidak ditemukan.</td>
            </tr>
            <tr v-for="item in paginatedItems" :key="item.id" class="hover:bg-slate-50/50 transition-colors group">
              ...
              <td class="px-6 py-4 text-sm font-bold text-slate-800">{{ item.kode_mapel }}</td>
              <td class="px-6 py-4 text-sm text-slate-600 font-medium">{{ item.nama_mapel }}</td>
    ...
    <BaseModal 
      :show="showForm" 
      :title="isEditMode ? 'Edit Mata Pelajaran' : 'Tambah Mapel Baru'"
      :isLoading="isSaving"
    ...
    >
      <div class="space-y-4">
        <div>
          <label class="block text-sm font-semibold text-slate-700 mb-1.5">Kode Mata Pelajaran</label>
          <input v-model="form.kode_mapel" type="text" placeholder="Contoh: BIND, MTK, IPA" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 text-sm">
        </div>
        <div>
          <label class="block text-sm font-semibold text-slate-700 mb-1.5">Nama Mata Pelajaran</label>
          <input v-model="form.nama_mapel" type="text" placeholder="Masukkan nama mata pelajaran" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 text-sm">
        </div>
      </div>
    </BaseModal>
    ...
    <ConfirmModal 
      ...
      :message="deleteTargetId ? 'Apakah Anda yakin ingin menghapus mata pelajaran ini?' : `Apakah Anda yakin ingin menghapus ${selectedIds.length} mata pelajaran yang dipilih?`"
