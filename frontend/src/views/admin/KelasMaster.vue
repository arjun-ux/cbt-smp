...
const fetchData = async () => {
  ...
  try {
    ...
    alertStore.showAlert("Gagal memuat data kelas", "error")
  } finally {
...
const filteredItems = computed(() => {
  ...
  return items.value.filter(i => 
    i.nama_kelas.toLowerCase().includes(q) || i.tingkat.toLowerCase().includes(q)
  )
})
...
const saveItem = async () => {
  isSaving.value = true
  try {
    const url = isEditMode.value ? `/api/admin/kelas/${editId.value}` : '/api/admin/kelas'
    ...
    if(res.ok) {
      alertStore.showAlert(isEditMode.value ? "Data diperbarui" : "Kelas berhasil ditambahkan", "success")
...
const executeDelete = async () => {
  ...
  try {
    if (deleteTargetId.value) {
      const res = await fetch(`/api/admin/kelas/${deleteTargetId.value}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${authStore.token}` }
      })
      if (res.ok) {
...
<template>
  <div class="space-y-6 pb-12">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-100 gap-4">
      <div>
        <h3 class="text-xl font-bold text-slate-800">Manajemen Kelas</h3>
        <p class="text-sm text-slate-500 mt-1">Kelola data tingkat dan pembagian kelas</p>
      </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      <div class="p-4 border-b border-slate-50 bg-slate-50/30 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div class="relative w-full sm:w-80">
          ...
          <input v-model="searchQuery" type="text" placeholder="Cari tingkat atau nama kelas..." class="block w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 transition-all">
        </div>
        ...
        <table class="min-w-full divide-y divide-slate-100">
          <thead class="bg-slate-50">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Tingkat</th>
          ...
          <tbody class="bg-white divide-y divide-slate-50">
            ...
            <tr v-else-if="filteredItems.length === 0">
              <td colspan="4" class="px-6 py-12 text-center text-slate-400 italic">Data kelas tidak ditemukan.</td>
            </tr>
            <tr v-for="item in paginatedItems" :key="item.id" class="hover:bg-slate-50/50 transition-colors group">
              ...
              <td class="px-6 py-4 text-sm font-bold text-slate-800">{{ item.tingkat }}</td>
              <td class="px-6 py-4 text-sm text-slate-600 font-medium">{{ item.nama_kelas }}</td>
    ...
    <BaseModal 
      :show="showForm" 
      :title="isEditMode ? 'Edit Kelas' : 'Tambah Kelas Baru'"
      :isLoading="isSaving"
    ...
    >
      <div class="space-y-4">
        <div>
          ...
          <select v-model="form.tingkat" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 text-sm">
        ...
        <div>
          ...
          <input v-model="form.nama_kelas" type="text" placeholder="Contoh: 7-A, 8-B" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 text-sm">
        </div>
      </div>
    </BaseModal>
    ...
    <ConfirmModal 
      ...
      title="Hapus Kelas"
      :message="deleteTargetId ? 'Apakah Anda yakin ingin menghapus kelas ini?' : `Apakah Anda yakin ingin menghapus ${selectedIds.length} kelas yang dipilih?`"
