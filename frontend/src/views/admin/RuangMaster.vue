...
const fetchData = async () => {
  ...
  try {
    const res = await fetch('/api/admin/ruang', {
      headers: { 'Authorization': `Bearer ${authStore.token}` }
    })
    const data = await res.json()
    if(res.ok) items.value = data.data
  } catch (error) {
    alertStore.showAlert("Gagal memuat data ruang", "error")
  } finally {
...
const filteredItems = computed(() => {
  ...
  return items.value.filter(i => 
    i.nama_ruang.toLowerCase().includes(q) || (i.kode_ruang && i.kode_ruang.toLowerCase().includes(q))
  )
})
...
const openAdd = () => {
  ...
  editId.value = null
  form.value = { kode_ruang: '', nama_ruang: '' }
  showForm.value = true
}
...
const saveItem = async () => {
  isSaving.value = true
  try {
    const url = isEditMode.value ? `/api/admin/ruang/${editId.value}` : '/api/admin/ruang'
    ...
    if(res.ok) {
      alertStore.showAlert(isEditMode.value ? "Data diperbarui" : "Ruang berhasil ditambahkan", "success")
...
const executeDelete = async () => {
  ...
  try {
    if (deleteTargetId.value) {
      const res = await fetch(`/api/admin/ruang/${deleteTargetId.value}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${authStore.token}` }
      })
      if (res.ok) {
      ...
      for (const id of selectedIds.value) {
        const res = await fetch(`/api/admin/ruang/${id}`, {
          method: 'DELETE',
          headers: { 'Authorization': `Bearer ${authStore.token}` }
        })
        if (res.ok) {
...
<template>
  <div class="space-y-6 pb-12">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-100 gap-4">
      <div>
        <h3 class="text-xl font-bold text-slate-800">Manajemen Ruang</h3>
        <p class="text-sm text-slate-500 mt-1">Kelola daftar ruangan pelaksanaan ujian</p>
      </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      <div class="p-4 border-b border-slate-50 bg-slate-50/30 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div class="relative w-full sm:w-80">
          ...
          <input v-model="searchQuery" type="text" placeholder="Cari nama ruang..." class="block w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 transition-all">
        </div>
        ...
        <div v-else class="text-xs font-bold text-slate-400 bg-slate-100 px-3 py-1.5 rounded-lg uppercase tracking-wider">
          Total: {{ filteredItems.length }} Ruang
        </div>
      </div>
        ...
        <table class="min-w-full divide-y divide-slate-100">
          <thead class="bg-slate-50">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Kode Ruangan</th>
              <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Nama Ruangan</th>
          ...
          <tbody class="bg-white divide-y divide-slate-50">
            ...
            <tr v-else-if="filteredItems.length === 0">
              <td colspan="4" class="px-6 py-12 text-center text-slate-400 italic">Data ruang tidak ditemukan.</td>
            </tr>
            <tr v-for="item in paginatedItems" :key="item.id" class="hover:bg-slate-50/50 transition-colors group">
              ...
              <td class="px-6 py-4 text-sm font-bold text-slate-800">{{ item.kode_ruang }}</td>
              <td class="px-6 py-4 text-sm text-slate-600 font-medium">{{ item.nama_ruang }}</td>
    ...
    <BaseModal 
      :show="showForm" 
      :title="isEditMode ? 'Edit Ruang' : 'Tambah Ruang Baru'"
      :isLoading="isSaving"
    ...
    >
      <div class="space-y-4">
        <div>
          <label class="block text-sm font-semibold text-slate-700 mb-1.5">Kode Ruangan</label>
          <input v-model="form.kode_ruang" type="text" placeholder="Contoh: R-01, LAB-01" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 text-sm">
        </div>
        <div>
          <label class="block text-sm font-semibold text-slate-700 mb-1.5">Nama Ruangan</label>
          <input v-model="form.nama_ruang" type="text" placeholder="Masukkan nama ruangan" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 text-sm">
        </div>
      </div>
    </BaseModal>
    ...
    <ConfirmModal 
      ...
      title="Hapus Ruangan"
      :message="deleteTargetId ? 'Apakah Anda yakin ingin menghapus ruangan ini?' : `Apakah Anda yakin ingin menghapus ${selectedIds.length} ruangan yang dipilih?`"
